import { PageView, VisitorSession, IPageView, IVisitorSession } from "@/models/Analytics";
...
class AnalyticsStore {
    ...
    private parseUserAgent(userAgent: string): { device: 'desktop' | 'mobile' | 'tablet'; browser: string; os: string } {
    ...
    // Track page view
    async trackPageView(page: string, visitorInfo: VisitorInfo, sessionId?: string): Promise<void> {
        try {
            await connectToDatabase();
            
            const { device, browser, os } = this.parseUserAgent(visitorInfo.userAgent);
            ...
            const existingSession = await VisitorSession.findOne({ sessionId: currentSessionId });
    ...
    async getAnalyticsData(): Promise<AnalyticsData> {
        try {
            ...
            const averageSessionDuration = sessionsWithDuration[0]?.avgDuration || 0;
            
            // Bounce rate (sessions with only 1 page view)
            const totalSessions = await VisitorSession.countDocuments();
            const bounceSessions = await VisitorSession.countDocuments({ pageViews: 1 });
            ...
            const deviceBreakdown = deviceStats.map(stat => ({
                device: stat._id,
                count: stat.count,
                percentage: totalVisitors > 0 ? (stat.count / totalVisitors) * 100 : 0
            }));
            ...
            for (let i = 6; i >= 0; i--) {
                ...
                const dayVisitors = dailyVisitorData.find(d => d._id === dateStr);
