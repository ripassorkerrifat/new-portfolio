...
const ProjectsPage: React.FC = () => {
    ...
    const fetchAllProjects = async (category: string = "all") => {
        try {
            setLoading(true);
            const url = `/api/projects/public?category=${category}&limit=50`; // Fetch more projects for dedicated page
    ...
    // Load projects on component mount and filter change
    useEffect(() => {
        fetchAllProjects(activeFilter);
    }, [activeFilter]);
            ...
            <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
                ...
                {error && (
                    <div className="text-center py-20">
                        ...
                        <button
                            onClick={() => fetchAllProjects(activeFilter)}
                            className="px-8 py-3 bg-[var(--primary-color)] text-white rounded-xl hover:bg-[var(--primary-color)]/80 transition-colors font-semibold">
                            Try Again
                        </button>
                    </div>
                )}
