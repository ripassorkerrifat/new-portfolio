    ...
    >
      ...
      <div className="relative h-48 overflow-hidden">
        ...
        <button
          ...
          className="absolute top-4 left-4 w-10 h-10 bg-black/50 backdrop-blur-sm rounded-full flex items-center justify-center text-white hover:bg-black/70 transition-all duration-300 transform hover:scale-110"
          title="View Gallery"
        >
          🖼️
        </button>
        ...
        {project.featured && (
          <div className="absolute top-4 right-4">
            <span className="bg-gradient-to-r from-[var(--primary-color)] to-[var(--secondary-color)] text-white px-3 py-1 rounded-full text-xs font-bold shadow-lg">
              ⭐ Featured
            </span>
          </div>
        )}
      ...
      <div className="relative p-6">
        {/* Project title */}
        <h4 className="text-xl font-bold text-[var(--text-primary)] mb-3 group-hover:text-[var(--primary-color)] transition-colors duration-300">
          {project.title}
        </h4>
        
        {/* Project description */}
        <p className="text-[var(--text-secondary)] text-sm mb-5 leading-relaxed line-clamp-2 min-h-[2.5rem]">
          {project.description}
        </p>
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {project.technologies.slice(0, 3).map((tech) => (
            <span
              key={tech}
              className="bg-gradient-to-r from-[var(--primary-color)]/10 to-[var(--secondary-color)]/10 text-[var(--text-primary)] px-3 py-1.5 rounded-full text-xs font-medium border border-[var(--border-color)]/30 hover:border-[var(--primary-color)]/50 transition-all duration-300 hover:scale-105"
            >
              {tech}
            </span>
          ))}
          {project.technologies.length > 3 && (
            <span className="text-[var(--text-secondary)] text-xs px-3 py-1.5 bg-[var(--secondary-bg)]/30 rounded-full border border-[var(--border-color)]/30">
              +{project.technologies.length - 3}
            </span>
          )}
        </div>
