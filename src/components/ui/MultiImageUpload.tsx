        ...
        >
          ...
          <div className="text-center">
            <div className="w-12 h-12 bg-[var(--primary-color)]/10 rounded-full flex items-center justify-center mx-auto mb-3">
              <FaPlus className="text-[var(--primary-color)]" size={20} />
            </div>
            <p className="text-[var(--text-primary)] font-medium mb-1">Add Gallery Images</p>
            <p className="text-[var(--text-secondary)] text-sm">Click to browse or drag and drop multiple images</p>
            <p className="text-[var(--text-secondary)] text-xs mt-1">
              PNG, JPG, GIF up to {maxSize}MB each • {value.length}/{maxFiles} images
            </p>
          </div>
        </div>
      )}
            ...
            <div key={index} className="relative group">
              ...
              <div className="absolute bottom-1 left-1 bg-black/50 backdrop-blur-sm rounded px-2 py-1">
                <span className="text-white text-xs">
                  {file instanceof File ? 'Ready' : 'Uploaded'}
                </span>
              </div>
            </div>
          ))}
